...
function ProjectsPage() {
  ...
  const viewedProject = viewTarget
    ? projects.find((p) => p.id === viewTarget)
    : null;
      ...
      {viewedProject && (
        <div className="space-y-4">
          <button
            ...
            className="flex items-center gap-1 text-sm text-brand-400 hover:text-brand-300 transition-colors cursor-pointer"
          >
          ...
          <Card>
              ...
              {canManage && (
                <div className="flex gap-2">
                  <Button
                    variant="ghost"
                    size="sm"
                    onClick={() => openEdit(viewedProject)}
                  >
            ...
            <div className="mt-4 flex gap-4 text-xs text-slate-500">
              <span>
                Created: {new Date(viewedProject.createdAt).toLocaleDateString()}
              </span>
              <span>
                Updated: {new Date(viewedProject.updatedAt).toLocaleDateString()}
              </span>
            </div>
          </Card>
        </div>
            ...
            <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
              {projects.map((project) => (
                <Card
                  key={project.id}
                  hover
                  onClick={() => setViewTarget(project.id)}
                >
                  <div className="flex items-start justify-between">
